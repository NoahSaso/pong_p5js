function Ball() {
  ...
  this.update = function() {
    ...
    if (this.y - this.size <= 1 || this.y + this.size >= height - 1) {
      this.vy *= -1;
    }
    ...
    if (this.x - this.size / 2 <= leftPaddle.x - leftPaddle.width / 2 - 10 || this.x + this.size / 2 >= rightPaddle.x + rightPaddle.width / 2 + 10) {
      this.reset();
    }
  }
